...
public:
    ...
    static Position* interpolatePosition(Position & p1, Position & p2, uint64_t timestamp) {
        ...
        return new Position(interpLat, interpLon, interpAlt, timestamp);
    }

    static Attitude* interpolateAttitude(Attitude & a1, Attitude & a2,uint64_t timestamp) {
        double interpRoll = linearInterpolation(a1.getRoll(), a2.getRoll(), timestamp, a1.getTimestamp(), a2.getTimestamp());
        double interpPitch = linearInterpolation(a1.getPitch(), a2.getPitch(), timestamp, a1.getTimestamp(), a2.getTimestamp());
        double interpHeading = linearHeadingRadiansInterpolation(a1.getHeading(), a2.getHeading(), timestamp, a1.getTimestamp(), a2.getTimestamp());
        return new Attitude(interpRoll, interpPitch, interpHeading, timestamp);
    }
    ...
    static double linearHeadingInterpolation(double psi1, double psi2, uint64_t t, uint64_t t1, uint64_t t2) {
    ...
    static double linearHeadingRadiansInterpolation(double psi1, double psi2, uint64_t t, uint64_t t1, uint64_t t2) {
        double interpDegrees = linearHeadingInterpolation(psi1*180.0/M_PI, psi2*180.0/M_PI, t, t1, t2);
