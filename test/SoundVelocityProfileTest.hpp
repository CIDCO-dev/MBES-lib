...
TEST_CASE("Read the sound velocity profile time, latitude, longitude")
{
    ...
    double lat = 0;
    double lon = 0;
    bool valide = svp.readSectionHeader(text,time,lat,lon);
    REQUIRE(valide);
    REQUIRE(time==0);
    REQUIRE(lat==-1);
    REQUIRE(lon==-1);
...
TEST_CASE("Use readSectionHeader with a wrong row")
{
    SoundVelocityProfile svp = SoundVelocityProfile();
    ...
    double lat = 0;
    double lon = 0;
    bool valide1 = svp.readSectionHeader(text,time,lat,lon);
    text = "Section 1970-1 00:0 South 1:0:0 West 1:0:0";
    bool valide2 = svp.readSectionHeader(text,time,lat,lon);
    text = "Section 1970-1 0:0:0 Suth 1:0:0 West 1:0:0";
    bool valide3 = svp.readSectionHeader(text,time,lat,lon);
    text = "Section 1970-1 0:0:0 South 1:00 West 1:0:0";
    bool valide4 = svp.readSectionHeader(text,time,lat,lon);
    text = "Section 1970-1 0:0:0 South 1:0:0 Wst 1:0:0";
    bool valide5 = svp.readSectionHeader(text,time,lat,lon);
